...
Rust Book 将常用数据类型分为两大类：**标量类型（Scalar）** 和 **复合类型（Compound）**。本文另外介绍用于表示发散控制流的特殊类型 `!`。
...
| `i32` | 32 | −(2³¹) | 2³¹ − 1 | **无类型约束时的默认整数类型** |
| `i64` | 64 | −(2⁶³) | 2⁶³ − 1 | 有符号 64 位 |
| `i128` | 128 | −(2¹²⁷) | 2¹²⁷ − 1 | 有符号 128 位 |
| `isize` | arch* | `isize::MIN` | `isize::MAX` | 与目标架构的指针宽度一致 |
| `u8` | 8 | 0 | 255 | 无符号 8 位 |
| `u16` | 16 | 0 | 65_535 | 无符号 16 位 |
| `u32` | 32 | 0 | 2³² − 1 | 无符号 32 位 |
| `u64` | 64 | 0 | 2⁶⁴ − 1 | 无符号 64 位 |
| `u128` | 128 | 0 | 2¹²⁸ − 1 | 无符号 128 位 |
| `usize` | arch* | 0 | `usize::MAX` | 与目标架构的指针宽度一致，常用于索引和长度 |

> **arch\*** 表示大小取决于计算机架构（32 位或 64 位）。

有符号 `n` 位整数的取值范围为 `−2^(n−1) ..= 2^(n−1)−1`，无符号 `n` 位整数的取值范围为 `0 ..= 2^n−1`。这里的 `^` 表示数学上的乘方；在 Rust 表达式中，`^` 是按位异或运算符。无符号整数可以表示包括 `0` 在内的非负整数。

在 32 位目标上，`isize`/`usize` 的范围分别与 `i32`/`u32` 相同；在 64 位目标上，分别与 `i64`/`u64` 相同。代码中应优先使用 `isize::MIN`、`isize::MAX` 和 `usize::MAX` 获取当前目标的实际边界。
...
| `f64` | 64 | ~15-16 位有效数字 | **无类型约束时的默认浮点类型**，双精度 |
...
`!` 是一种特殊的无人居住类型（uninhabited type），表示计算**永远不会正常产生值**，也称为“never type”。它不属于 Rust Book 列出的四种标量类型。

| 特性 | 说明 |
|------|------|
| 类型表示 | `!` |
| 值 | **不存在任何值** |
| 大小 | 不适用：该类型不存在任何值，无法被正常实例化 |
...
复合类型将多个值组合成一个类型。单元类型 `()` 是不包含任何元素的空元组。
...
| 特殊类型 | `!` | 不适用（不存在任何值） | — | 发散控制流（panic/loop） |
| 单元 | `()` | 0 字节 | — | 无返回值 |
| 元组 | `(T1, T2, ...)` | 各元素之和 | — | 多值返回 |
