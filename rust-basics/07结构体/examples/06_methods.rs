...
fn main() {
    ...
    println!(
        "  5x5   is_square = {}, is_tall = {}",
        square_like.is_square(),
        square_like.is_tall()
    );
    println!(
        "  2x10  is_square = {}, is_tall = {}",
        tall.is_square(),
        tall.is_tall()
    );
    println!(
        "  10x2  is_square = {}, is_tall = {}",
        wide.is_square(),
        wide.is_tall()
    );
    ...
    // 调用 &self 方法：值 / 引用都可以
    r.area();
    // 刻意演示 (&r).area() 也合法（&self 方法接受引用调用），clippy 会建议去掉 &
    ...
    // ─────────────────────────────────────────
    println!(
        "  · {:<14} → 只读访问，最常用（读字段、计算派生值、比较）",
        "&self"
    );
    println!(
        "  · {:<14} → 可变访问，修改字段（mutations，需要调用者是 mut）",
        "&mut self"
    );
    println!(
        "  · {:<14} → 消费自身，转换形态（into_xxx，结束当前实例）",
        "self"
    );
