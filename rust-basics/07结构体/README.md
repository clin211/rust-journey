...
```text
  散装元组（痛苦）                      结构体（舒服）
  ─────────────────────────────        ─────────────────────────────
  (u64, String, String, bool, u32)     struct User {
    .0   .1     .2      .3    .4         id:     u64,     ← 每个字段有名字
     ↑    ↑      ↑       ↑     ↑         name:   String,
     这些字段分别是谁？                    email:  String,
     顺序一错就全乱                        active: bool,
                                          age:    u32,
                                        }

  同结构不同义：编译器分不清             新类型 = 新身份：编译器能拦截混用
  ┌─────────────────────────────┐      ┌───────────────┐    ┌───────────────┐
  │ (1, "a", "b", true, 30)     │      │ struct User { │    │ struct Order {│
  │ ↑ User 还是 Order？          │      │   id, name,   │ ≠  │   id, name,   │
  │ 函数签名一样 → bug 钻进业务  │      │   email, ...  │    │   email, ...  │
  └─────────────────────────────┘      │ }             │    │ }             │
                                       └───────────────┘    └───────────────┘
                                       User 和 Order 即使字段完全一样,
                                       在类型系统里也是两种独立类型
```
...
```text
  ...
  不存在的语法:                         想让单个字段在不可变实例里"可变":
  ─────────────────────                 ─────────────────────────────
  struct S {                            use std::cell::Cell;
    mut age: u32,      // ❌              struct S {
    name: String,                           age: Cell<u32>,    // ← 内部可变性
  }                                         name: String,
                                          }
  // 整个语言都不支持
                                          let s = S { .. };   // 不 mut
...
│                                                                            │
│   src: User            let u_a = User { age:99, active:false, ..src }      │
│   ┌──────────────────┐        │                                            │
│   │ username: "src"  │ ──move─┤                                            │
│   │ email:    "s@x"  │ ──move─┤                                            │
│   │ age:      10     │ ──copy─┤ (但会被 99 覆盖)                           │
│   │ active:   true   │ ──copy─┤ (但会被 false 覆盖)                        │
│   └──────────────────┘        │                                            │
│                               ▼                                            │
│                        u_a: User                                           │
│                        ┌──────────────────┐                                │
...
│                                                                            │
│   src2: User           let u_b = User { username:"new", email:"n@x", ..src2│
│   ┌──────────────────┐                                                     │
│   │ username: "src2" │ ← ✗ 不需要 move，已经被显式覆盖                      │
│   │ email:    "s2@x" │ ← ✗ 不需要 move，已经被显式覆盖                      │
│   │ age:      20     │ ──copy──→  u_b.age    = 20                          │
│   │ active:   true   │ ──copy──→  u_b.active = true                        │
│   └──────────────────┘                                                     │
│                                                                            │
│   之后 src2 的状态:                                                        │
│   ┌──────────────────┐                                                     │
...
```text
  ...
  ─────────────────────────────────────────────
  f64:      ┌──────────────────────┐
            │ 64 bit 浮点 (8 bytes) │   size_of::<f64>()    == 8
            └──────────────────────┘

  Meters(f64): ┌──────────────────────┐
               │ 64 bit 浮点 (8 bytes) │   size_of::<Meters>() == 8
               └──────────────────────┘
               (布局与 f64 完全相同)
...
│   fn area(&self) -> u32                                         │
│                                                                 │
│     caller            │ 方法调用期间 │         caller            │
│   ┌──────────┐       │              │       ┌──────────┐        │
│   │Rectangle │──&r──▶│  area(&r)    │       │Rectangle │ ✅     │
│   │ (保留)   │       │              │       │ (仍然可用)│        │
│   └──────────┘       │              │       └──────────┘        │
│                      └──────────────┘                           │
│   ✓ 调用前后 Rectangle 都属于调用方, 完好无损                    │
│   ✓ 调用期间其它地方也能并发 &r (共享读)                         │
└─────────────────────────────────────────────────────────────────┘
...
│     caller            │ 方法调用期间 │         caller            │
│   ┌──────────┐       │              │       ┌──────────┐        │
│   │Rectangle │──&mut▶│ scale(&mut r)│       │Rectangle │ ✅     │
│   │ (保留)   │       │              │       │ (已被改过)│        │
│   └──────────┘       │              │       └──────────┘        │
│                      └──────────────┘                           │
...
│     caller            │ 方法调用后    │        caller            │
│   ┌──────────┐       │              │       ┌──────────┐        │
│   │Rectangle │─move─▶│into_square(r)│─────▶ │Rectangle │ (新)   │
│   │ 🚫作废    │       │   (持有 r)   │       │(返回值)   │        │
│   └──────────┘       └──────────────┘       └──────────┘        │
...
```text
  ...
  │ ┌──────────────┐ │                  │ ┌──────────────┐ │
  │ │ r: &str      │ │                  │ │ r: &str      │ │
  │ │   ptr ─→[0x10│ │                  │ │   ptr ─→[0x10│ │← 指向老地址
  │ └──────────────┘ │                  │ └──────────────┘ │
  └──────────────────┘                  └──────────────────┘
                                         ↑
                                         r 还是指向 0x1000 的 s 字段,
                                         但那块栈已经作废了 → 悬垂

  更根本的问题:
    · Rust 的生命周期系统无法表达 "r 的借用源就是 s" 这种自指关系
      (因为 's 的生命周期' 和 'r 的生命周期' 是同一个对象)
    · Drop 顺序如果是 [s, r] 还是 [r, s], 结果都可能导致不安全
```
...
```text
  Option<&T> 的内存布局:
  ┌──────────────────────────┐
  │   8 bytes (一个 ptr 大小) │      size_of::<Option<&T>>() == size_of::<&T>()
  └──────────────────────────┘                           == 8
      │
      ├── 非 0 地址 → Some(&T), 其值就是那个地址
      └── 0 (null)  → None
...
```text
  ...
  │  HTTP  ──┐                                      │
  │  CLI   ──┼──▶ DTO ──▶ 校验 ──▶ 领域模型 ──▶ 持久化   │
  │  RPC   ──┘        (Request)  (Domain Model) (ORM)│
  │                                            │    │
  │                                            ▼    │
  │                                          数据库 / 文件 / 消息队列 │
  │                                                 │
  │  DTO:          pub 字段 + Serde derive          │
  │  Domain Model: 私有字段 + 方法 + 不变量         │
  │  Persistence:  带 id、时间戳、版本号等元信息    │
  │                                                 │
  └─────────────────────────────────────────────────┘
```
